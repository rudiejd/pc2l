...
Exception::Exception(const std::string &errorMessage,
                     const std::string &suggestion, const std::string &fileName,
  ...
  if (!fileName.empty()) {
    ...
    fullInfo +=
        "\n\tAt: " + fileName + "(line: " + std::to_string(lineNumber) + ")";
  }
}

std::ostream &operator<<(std::ostream &os, const pc2l::Exception &exp) {
  return (os << exp.what());
}
