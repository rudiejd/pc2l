...
ArgParser::ArgParser(const ArgRecord validArguments[], const std::string &info)
    : info(info) {
  addValidArguments(validArguments);
}

void ArgParser::addValidArguments(const ArgRecord validArguments[]) {
  ...
  while (true) {
    if (validArguments[scannedArgumentCount].type == INVALID) {
      ...
      if (argRecords.size() > MAXIMUM_ARGUMENTS) {
        std::cerr << "Error: There seem to be more than " << MAXIMUM_ARGUMENTS
                  << " arguments specified" << std::endl
                  << "Please check your code for the "
                  << "{\"\", \"\", NULL, ArgType::INVALID}"
                  << " marker or increase MAXIMUM_ARGUMENTS" << std::endl;
        exit(100);
      }
    }
  }
...
void ArgParser::parseArguments(int &argc, char *argv[], bool caxoe) {
    ...
    for (int match = 0; ((match < NumArgs) && (argument < argc)); match++) {
      ...
      if ((argRec.type != BOOLEAN) && (argRec.type != STRING_LIST)) {
        if ((argument + 1 == argc) || ((argv[argument + 1][0] == '-') &&
                                       (argv[argument + 1][1] == '-'))) {
          std::cerr << "Parameter for " << argRec.command << " (" << argRec.help
                    << ")"
                    << " at position " << match << " of " << NumArgs
                    << " is missing" << std::endl;
          exit(101);
        }
      }
      ...
      case LONG:
        removeArgument(argument, argc, argv);
        *(reinterpret_cast<long *>(argRec.data)) = atol(argv[argument]);
        removeArgument(argument, argc, argv);
        break;

      case STRING_LIST: {
        removeArgument(argument, argc, argv);
        StringList *strList = reinterpret_cast<StringList *>(argRec.data);
...
void ArgParser::removeArgument(int removeIdx, int &argc, char *argv[]) {
  memmove(argv + removeIdx, argv + removeIdx + 1,
          sizeof(char *) * (argc - removeIdx - 1));
  argv[--argc] = NULL;
}

bool ArgParser::checkRemainingArguments(int argc, char *argv[], bool caxoe,
                                        bool caxoa) {
...
std::ostream &operator<<(std::ostream &os, ArgParser &ap) {
  ...
  for (size_t i = 0; (i < ap.argRecords.size()); i++) {
    ...
    if (argRec.type != ArgParser::MAIN_MESSAGE) {
      ap.printArg(os, IndentStr, maxLen - argRec.command.size() + numSpaces,
                  argRec);
    } else {
...
std::string ArgParser::getValue(const ArgRecord &argRec) {
...
void ArgParser::printArg(std::ostream &os, const std::string &indentStr,
  ...
  const size_t spaceRemaining =
      maxWidth - indentStr.size() - argRec.command.size() - numSpaces - 1;
  const std::string defaultValue = getValue(argRec);
  std::string help =
      argRec.help + (!defaultValue.empty() ? " [" + defaultValue + "]" : "");
  while (!help.empty()) {
