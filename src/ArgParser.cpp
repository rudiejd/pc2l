          ...
          if (argRecords.size () > MAXIMUM_ARGUMENTS)
            {
              std::cerr << "Error: There seem to be more than "
                        << MAXIMUM_ARGUMENTS << " arguments specified"
                        << std::endl
      ...
      for (int match = 0; ((match < NumArgs) && (argument < argc)); match++)
        {
          ...
          switch (argRec.type)
            ...
            case STRING:
              removeArgument (argument, argc, argv);
              *(reinterpret_cast<std::string *> (argRec.data))
                  = argv[argument];
              removeArgument (argument, argc, argv);
              break;
            ...
            case FLOAT:
              removeArgument (argument, argc, argv);
              *(reinterpret_cast<float *> (argRec.data))
                  = (float)atof (argv[argument]);
              removeArgument (argument, argc, argv);
              break;

            case DOUBLE:
              removeArgument (argument, argc, argv);
              *(reinterpret_cast<double *> (argRec.data))
                  = atof (argv[argument]);
              removeArgument (argument, argc, argv);
              break;
...
bool
ArgParser::checkRemainingArguments (int argc, char *argv[], bool caxoe,
                                    bool caxoa)
{
...
std::ostream &
operator<< (std::ostream &os, ArgParser &ap)
{
      ...
      if (argRec.type != ArgParser::MAIN_MESSAGE)
        {
          ap.printArg (os, IndentStr,
                       maxLen - argRec.command.size () + numSpaces, argRec);
        }
      else
        {
